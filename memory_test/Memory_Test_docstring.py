...
def display_intro(sentences, durations):
    ...
    for idx in range(len(sentences)):
        ...
        if idx==1:
            ...
            #give an input box for users to enter their IDs
            ID= input("")
            data_dict["ID"] = ID
            ...
            #assigned a duration for each sentence to be displayed before cleared, which is listed above in the duration list
            time.sleep(duration)
            clear_output(wait=True)

#all questions defined as a dictionary, attributed with a variable name. Varaibles are parameters for functions run in the main.           
...
def analyze_graph(graph_name, questions, boolean):
  ...
  # Ask the user a question in multiple choice format
    ...
    score = 0
    #Loop through each question in the 'questions' list, with an enumeration starting at 1
    for question_num, question in enumerate(questions, start=1):
        ...
        if answer == questions[question]:
            score += 1
        clear_output()
        #use boolean value as parameter of the function to run display_grid function for the part of questions asking about positional information.
        if boolean == False:
            display_grid(image)
...
def main():
    """
    This function combined the above functions to giveout the main part of Memory Test.
    By accepting participan't permit, introduction is displayed and the start of the test.
    Graphs presented by in sequence, followed with related questions.
    User's responses are collected and scored.
    Conclusion and boxplot show for user to do comparison with class dataset.
    Information is collected and sent to Google Form for analysis.
    """
    ...
    text_input = input("")
    #only runs the test if user consent to send data to google form
    if text_input == "yes":
        #show introduction
        ...
        clear_output(wait=True)
        #question with the picture, specified by a boleen value of False
        ...
        #calculate the total score of the test
        ...
        #display the conclusions and user output compared to class dataset
